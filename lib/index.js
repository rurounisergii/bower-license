...
exports.printCSV = function(sorted, project){
   
    results = JSON.parse(JSON.stringify(sorted));
    appName = project.substring(0, project.length -1); 
    ...
    for (var key in results) {
        ...
        if (results.hasOwnProperty(key)) {
            line += key;
            line += "," + results[key].version + ',' + results[key].licenses;
        }
         str += line + '\r\n';   
    }
